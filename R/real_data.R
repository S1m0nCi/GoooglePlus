...
func_main <- function(NUM, ITER, method, penalties)
{
    ...
    for (penalty in penalties)
    {
        ...
        for (k in (NUM * ITER + 1):((NUM + 1) * ITER))
        {
            ...
            for (i in 1:5)
            {
                cv.result <- createFolds(data$docvisits, k = 5)
                train.idx <- cv.result[cv.result$fold != i, (k + 1)]
                train <- data[train.idx, ]
                test <- data[-train.idx, ]

                ptm <- proc.time()
                fit <- method(data = train, yvar = yvar, xvars = xvars, zvars = zvars, group = group, dist = "poisson", penalty = penalty, samegrp.overlap = TRUE)
